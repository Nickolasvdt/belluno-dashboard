    ...
    <div className="space-y-5">
      ...
      {temHoje ? (
        <div className="bg-white dark:bg-[#171411] rounded-2xl border border-cream-200 dark:border-white/[0.06] p-5 shadow-sm">
          ...
          <p className="text-4xl font-display font-bold text-gray-900 dark:text-gray-100 mb-5 tracking-tight">
            R$ {fmt(hoje.fechamento)}
          </p>
          <div className="grid grid-cols-3 gap-3 pt-4 border-t border-cream-200 dark:border-white/[0.05]">
            <div>
              <p className="text-[10px] text-gray-400 dark:text-zinc-500 uppercase tracking-wide mb-0.5">Inicial</p>
              <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">R$ {fmt(hoje.saldoInicial)}</p>
            </div>
            <div>
              <p className="text-[10px] text-gray-400 dark:text-zinc-500 uppercase tracking-wide mb-0.5">Entradas</p>
              <p className="text-sm font-semibold text-emerald-600 dark:text-emerald-400">+{fmt(hoje.entradas)}</p>
            </div>
            <div>
          ...
          {hoje.observacao && (
            <p className="text-xs text-gray-400 dark:text-zinc-500 mt-3 pt-3 border-t border-cream-200 dark:border-white/[0.05]">{hoje.observacao}</p>
          )}
        </div>
      ) : (
                    ...
                    {confirming ? (
                      ...
                      <>
                        <div className="text-right">
                          ...
                          <p className="text-[10px] text-gray-400 dark:text-zinc-500">inicial: {fmt(r.saldoInicial)}</p>
                        </div>
      ...
      <BottomSheet open={open} onClose={() => setOpen(false)} title={editId ? 'Editar Registro' : 'Novo Registro de Caixa'}>
        <form onSubmit={handleSubmit} className="space-y-3.5">
          ...
          <div className="px-3.5 py-3 bg-cream-100 dark:bg-zinc-800/60 rounded-xl">
            <p className="text-xs text-gray-500 dark:text-zinc-400 mb-0.5">Fechamento calculado</p>
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-400 dark:text-zinc-500 mt-1">Diferença entre caixa físico e fechamento calculado</p>
          </div>
          <div>
