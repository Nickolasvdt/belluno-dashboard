    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-5">
          ...
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 text-2xl font-bold leading-none">×</button>
        </div>
        {children}
      </div>
    </div>
...
export default function FechamentoPage() {
  ...
  async function handleContaSubmit(e: React.FormEvent) {
    e.preventDefault(); setSubmitting(true)
    try { await post('/api/fechamento/contas', contaForm); setContaForm(emptyConta()); fetchAll() }
    finally { setSubmitting(false) }
  }
  ...
  const thCls = 'px-3 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase'
  const tdCls = 'px-3 py-3 text-sm text-gray-900 dark:text-gray-100'
  ...
  const cancelBtnCls = 'flex-1 py-2 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg font-medium hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors'
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-6">
        <select value={mes} onChange={e => setMes(parseInt(e.target.value))} className="px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg text-sm focus:outline-none focus:ring-primary">
          {MONTHS.map((m, i) => <option key={i+1} value={i+1}>{m}</option>)}
        </select>
        <select value={ano} onChange={e => setAno(parseInt(e.target.value))} className="px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg text-sm focus:outline-none focus:ring-primary">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-5 gap-3 mb-6">
        {[
          ...
          { label: 'Pizzas / Ticket', value: `${totalPizzas} / R$ ${formatBRL(ticketMedio)}`, color: 'text-gray-700 dark:text-gray-300', sub: 'un. / por pizza' },
        ].map(card => (
          <div key={card.label} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-3 shadow-sm">
      ...
      <div className="flex gap-1 mb-4 border-b border-gray-200 dark:border-gray-700">
        {(['vendas', 'funcionarios', 'contas', 'insumos'] as Tab[]).map(t => {
          ...
          return (
            <button key={t} onClick={() => setTab(t)} className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${tab === t ? 'border-primary text-primary' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}`}>
              {labels[t]}
            </button>
          )
        })}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-5 mb-5 shadow-sm">
            ...
            <form onSubmit={handleVendaSubmit}>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
                ...
                <CurrencyInput label="Outros (99food, Keeta...)" value={vendaForm.outros} onChange={v => setVendaForm({ ...vendaForm, outros: v })} />
                <CurrencyInput label="Taxas de cartão" value={vendaForm.taxas} onChange={v => setVendaForm({ ...vendaForm, taxas: v })} />
                <div>
        ...
        <div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 overflow-hidden shadow-sm">
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-100 dark:divide-gray-700">
                <thead className="bg-gray-50 dark:bg-gray-700">
                  <tr>{['Data', 'Despesa', 'Valor', 'Status', 'Ações'].map(h => <th key={h} className={thCls}>{h}</th>)}</tr>
                </thead>
                <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                  {contas.map(c => (
                  ...
                  {contas.length === 0 && <tr><td colSpan={5} className="px-4 py-8 text-center text-sm text-gray-400 dark:text-gray-500">Nenhuma conta fixa registrada.</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
