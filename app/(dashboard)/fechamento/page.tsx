...
function sum(arr: number[]) { return round2(arr.reduce((acc, v) => round2(acc + v), 0)) }
...
const thCls = 'px-3 py-3 text-left text-xs font-medium text-gray-400 dark:text-zinc-500 uppercase whitespace-nowrap'
const tdCls = 'px-3 py-3 text-sm text-gray-800 dark:text-gray-100 whitespace-nowrap'
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-3 sm:p-4">
      <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-xl p-4 sm:p-6 w-full max-w-lg max-h-[calc(100dvh-2rem)] overflow-y-auto">
        <div className="flex items-center justify-between mb-5">
          ...
          <button onClick={onClose} className="w-9 h-9 flex items-center justify-center rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-zinc-800 text-2xl font-bold leading-none">×</button>
        </div>
        {children}
      </div>
    </div>
...
export default function FechamentoPage() {
  ...
  const emptyConta = () => ({ date: format(new Date(ano, mes - 1, 1), 'yyyy-MM-dd'), despesa: '', valor: 0, pago: false, diaVencimento: '' as any })
  ...
  async function handleContaSubmit(e: React.FormEvent) {
    e.preventDefault(); setSubmitting(true)
    try {
      await post('/api/fechamento/contas', { ...contaForm, diaVencimento: (contaForm as any).diaVencimento || null })
  ...
  async function handleContaEdit() {
    ...
    try { await put(`/api/fechamento/contas/${editConta.id}`, editConta); setEditConta(null); fetchAll() }
    finally { setSubmitting(false) }
  }
  ...
  const saveBtnCls = 'flex-1 py-2 bg-primary text-white rounded-lg font-medium text-sm hover:bg-primary-dark disabled:opacity-50 transition-colors'
  const cancelBtnCls = 'flex-1 py-2 border border-gray-200 dark:border-zinc-700 text-gray-600 dark:text-gray-300 rounded-lg font-medium text-sm hover:bg-gray-50 dark:hover:bg-zinc-800 transition-colors'
  const years = Array.from({ length: 5 }, (_, i) => now.getFullYear() - 2 + i)

  return (
    <div className="min-w-0">
      <h2 className="text-xl font-display font-semibold text-gray-800 dark:text-gray-100 mb-4">Fechamento Mensal</h2>

      {/* Mês/Ano selector */}
      <div className="flex flex-wrap items-center gap-2 mb-5">
        <select value={mes} onChange={e => setMes(parseInt(e.target.value))}
          className="px-3 py-2 border border-gray-200 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-primary w-full sm:w-auto">
        ...
        {[
          {
            ...
            value: `R$ ${formatBRL(receitaLiquida)}`,
            color: 'text-emerald-600 dark:text-emerald-400',
            sub: `Bruta R$ ${formatBRL(totalReceita)} · taxas R$ ${formatBRL(totalTaxas)}`,
          },
          {
            label: 'Total despesas',
            value: `R$ ${formatBRL(totalDespesa)}`,
            color: 'text-primary',
            sub: `CMV ${cmvPct.toFixed(1)}% · func R$ ${formatBRL(totalFunc)} · fixas R$ ${formatBRL(totalFixas)}`,
          },
          {
            label: lucroLiquido >= 0 ? 'Lucro líquido' : 'Prejuízo',
            value: `R$ ${formatBRL(Math.abs(lucroLiquido))}`,
            color: lucroLiquido >= 0 ? 'text-gray-900 dark:text-white' : 'text-red-600 dark:text-red-400',
            sub: receitaLiquida > 0 ? `Margem ${((lucroLiquido / receitaLiquida) * 100).toFixed(1)}%` : '—',
          },
          {
            label: 'Pizzas',
            value: String(totalPizzas),
            color: 'text-gray-700 dark:text-zinc-200',
            sub: `Ticket R$ ${formatBRL(ticketMedio)}`,
          },
        ].map(c => (
          <div key={c.label} className={`${card} p-4 min-w-0`}>
            <p className="text-[11px] font-medium uppercase tracking-wider text-gray-400 dark:text-zinc-500 mb-1">{c.label}</p>
            <p className={`text-base sm:text-lg font-bold break-words ${c.color}`}>{c.value}</p>
            <p className="text-[11px] text-gray-400 dark:text-zinc-600 mt-0.5 truncate">{c.sub}</p>
          </div>
        ))}
      </div>

      {/* ─── Abas ─── */}
      <div className="flex gap-0 -mx-3 sm:mx-0 px-3 sm:px-0 mb-4 border-b border-cream-200 dark:border-zinc-800 overflow-x-auto">
          ...
          return (
            <button key={t} onClick={() => setTab(t)}
              className={`px-4 py-2.5 text-sm font-medium border-b-2 whitespace-nowrap transition-colors ${
                tab === t ? 'border-primary text-primary' : 'border-transparent text-gray-400 dark:text-zinc-500 hover:text-gray-600 dark:hover:text-gray-300'
              }`}>
              {labels[t]}
            </button>
          )
      ...
      {tab === 'vendas' && (
        <div>
          <div className={`${card} p-4 sm:p-5 mb-4`}>
            <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-zinc-500 mb-4">Nova Venda</p>
            <form onSubmit={handleVendaSubmit}>
              <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
                ...
                <CurrencyInput label="Taxas" value={vendaForm.taxas} onChange={v => setVendaForm({ ...vendaForm, taxas: v })} />
                <div>
          ...
          <div className={`${card} overflow-hidden`}>
            <div className="overflow-x-auto">
              <table className="min-w-[980px] divide-y divide-cream-200 dark:divide-zinc-800">
                ...
                <tbody className="divide-y divide-cream-200 dark:divide-zinc-800">
                  {vendas.map(v => {
                    ...
                    return (
                      <tr key={v.id} className="hover:bg-gray-50 dark:hover:bg-zinc-800/60">
                        <td className={tdCls}>{format(new Date(v.date), 'dd/MM', { locale: ptBR })}</td>
                        {fechado ? (
                          <td colSpan={9} className="px-3 py-3 text-xs text-gray-400 dark:text-zinc-500">
                          ...
                          <>
                            ...
                            <td className={`${tdCls} font-semibold text-emerald-600 dark:text-emerald-400`}>R$ {formatBRL(bruto)}</td>
                            <td className={`${tdCls} text-amber-600 dark:text-amber-400`}>{v.taxas > 0 ? `R$ ${formatBRL(v.taxas)}` : '—'}</td>
                  ...
                  {vendas.length === 0 && (
                    <tr><td colSpan={12} className="px-4 py-8 text-center text-sm text-gray-400 dark:text-zinc-500">Nenhuma venda registrada.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
      ...
      {tab === 'funcionarios' && (
        <div>
          <div className={`${card} p-4 sm:p-5 mb-4`}>
            <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-zinc-500 mb-4">Novo Pagamento</p>
            <form onSubmit={handleFuncSubmit}>
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
                ...
                <div>
                  ...
                  <input type="text" value={funcForm.semana} onChange={e => setFuncForm({ ...funcForm, semana: e.target.value })} className={inputCls} placeholder="Semana 1" />
                </div>
          ...
          <div className={`${card} overflow-hidden`}>
            <div className="overflow-x-auto">
              <table className="min-w-[580px] divide-y divide-cream-200 dark:divide-zinc-800">
                <thead className="bg-cream-100 dark:bg-zinc-800">
                  <tr>{['Data', 'Nome', 'Semana/Ref.', 'Valor', 'Ações'].map(h => <th key={h} className={thCls}>{h}</th>)}</tr>
                </thead>
                <tbody className="divide-y divide-cream-200 dark:divide-zinc-800">
                  {funcionarios.map(f => (
                    <tr key={f.id} className="hover:bg-gray-50 dark:hover:bg-zinc-800/60">
                      <td className={tdCls}>{format(new Date(f.date), 'dd/MM', { locale: ptBR })}</td>
                      <td className={tdCls}>{f.nome}</td>
                      <td className={`${tdCls} text-gray-400 dark:text-zinc-500`}>{f.semana || '—'}</td>
                      <td className={`${tdCls} font-semibold text-primary`}>R$ {formatBRL(f.valor)}</td>
                      <td className={tdCls}>
                  ...
                  {funcionarios.length === 0 && <tr><td colSpan={5} className="px-4 py-8 text-center text-sm text-gray-400 dark:text-zinc-500">Nenhum pagamento registrado.</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
      ...
      {tab === 'contas' && (
        <div>
          <div className={`${card} p-4 sm:p-5 mb-4`}>
            <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-zinc-500 mb-4">Nova Conta Fixa</p>
            <form onSubmit={handleContaSubmit}>
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
                ...
                <div className="col-span-2 lg:col-span-2">
                  ...
                  <input type="text" required value={contaForm.despesa} onChange={e => setContaForm({ ...contaForm, despesa: e.target.value })} className={inputCls} placeholder="Aluguel, Luz, Sabesp..." />
                </div>
                <CurrencyInput label="Valor" value={contaForm.valor} onChange={v => setContaForm({ ...contaForm, valor: v })} required />
              </div>
              <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-3 mb-4">
                <div className="flex items-center gap-2">
                  <label className="text-sm text-gray-500 dark:text-zinc-400 whitespace-nowrap">Vence todo dia:</label>
                  <select
                    value={(contaForm as any).diaVencimento || ''}
                    onChange={e => setContaForm({ ...contaForm, ...(contaForm as any), diaVencimento: e.target.value || null })}
                    className="px-3 py-1.5 border border-gray-200 dark:border-zinc-700 dark:bg-zinc-900 dark:text-white rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-primary"
                  >
                  ...
                  {(contaForm as any).diaVencimento && (
                    <span className="text-xs text-primary font-medium bg-primary/10 px-2 py-0.5 rounded-full">Recorrente</span>
                  )}
                </div>
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-sm text-gray-600 dark:text-gray-300">Marcar como pago</span>
                </label>
              </div>
              <button type="submit" disabled={submitting} className="w-full sm:w-auto px-5 py-2 bg-primary text-white rounded-lg text-sm font-medium hover:bg-primary-dark disabled:opacity-50 transition-colors">
                {submitting ? 'Salvando...' : 'Adicionar'}
          ...
          <div className={`${card} overflow-hidden`}>
            <div className="overflow-x-auto">
              <table className="min-w-[700px] divide-y divide-cream-200 dark:divide-zinc-800">
                <thead className="bg-cream-100 dark:bg-zinc-800">
                  <tr>{['Data', 'Despesa', 'Vencimento', 'Valor', 'Status', 'Ações'].map(h => <th key={h} className={thCls}>{h}</th>)}</tr>
                </thead>
                <tbody className="divide-y divide-cream-200 dark:divide-zinc-800">
                  {contas.map(c => (
                    <tr key={c.id} className="hover:bg-gray-50 dark:hover:bg-zinc-800/60">
                      <td className={tdCls}>{format(new Date(c.date), 'dd/MM', { locale: ptBR })}</td>
                      <td className={tdCls}>{c.despesa}</td>
                      <td className={tdCls}>
                        {c.diaVencimento
                          ? <span className="text-xs font-medium text-primary bg-primary/10 px-2 py-0.5 rounded-full">Dia {c.diaVencimento}</span>
                          : <span className="text-gray-400 dark:text-zinc-600">—</span>}
                      </td>
                      <td className={`${tdCls} font-semibold text-primary`}>R$ {formatBRL(c.valor)}</td>
                      <td className={tdCls}>
                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${c.pago ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400' : 'bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400'}`}>
                  ...
                  {contas.length === 0 && <tr><td colSpan={6} className="px-4 py-8 text-center text-sm text-gray-400 dark:text-zinc-600">Nenhuma conta fixa registrada.</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
      ...
      {tab === 'insumos' && (
        <div>
          <div className={`${card} p-4 sm:p-5 mb-4`}>
            <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-zinc-500 mb-4">Novo Insumo / Compra</p>
            <form onSubmit={handleInsumoSubmit}>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-4">
                ...
                <div>
                  ...
                  <input type="text" required value={insumoForm.fornecedor} onChange={e => setInsumoForm({ ...insumoForm, fornecedor: e.target.value })} className={inputCls} placeholder="PMG, Sacolão, Lenha..." />
                </div>
          ...
          <div className={`${card} overflow-hidden`}>
            <div className="overflow-x-auto">
              <table className="min-w-[520px] divide-y divide-cream-200 dark:divide-zinc-800">
                <thead className="bg-cream-100 dark:bg-zinc-800">
                  <tr>{['Data', 'Fornecedor / Descrição', 'Valor', 'Ações'].map(h => <th key={h} className={thCls}>{h}</th>)}</tr>
                </thead>
                <tbody className="divide-y divide-cream-200 dark:divide-zinc-800">
                  {insumos.map(i => (
                    <tr key={i.id} className="hover:bg-gray-50 dark:hover:bg-zinc-800/60">
                      <td className={tdCls}>{format(new Date(i.date), 'dd/MM', { locale: ptBR })}</td>
                      <td className={tdCls}>{i.fornecedor}</td>
                      <td className={`${tdCls} font-semibold text-primary`}>R$ {formatBRL(i.valor)}</td>
                      <td className={tdCls}>
                  ...
                  {insumos.length === 0 && <tr><td colSpan={4} className="px-4 py-8 text-center text-sm text-gray-400 dark:text-zinc-500">Nenhum insumo registrado.</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
      ...
      {editVenda && (
        <Modal title="Editar Venda" onClose={() => setEditVenda(null)}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-5">
            ...
            <CurrencyInput label="Taxas" value={editVenda.taxas} onChange={v => setEditVenda({ ...editVenda, taxas: v })} />
            <div>
              <label className={labelCls}>Pizzas</label>
