...
export default function FechamentoPage() {
  ...
  const ticketMedio = totalPizzas > 0 ? round2(totalReceita / totalPizzas) : 0
  ...
  const inputCls = 'w-full px-3 py-2 border border-gray-300 dark:border-zinc-700 dark:bg-zinc-900 dark:text-white rounded-md focus:outline-none focus:ring-primary focus:border-primary text-sm'
  const labelCls = 'block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1'
  const thCls = 'px-3 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase whitespace-nowrap'
  const tdCls = 'px-3 py-3 text-sm text-gray-900 dark:text-gray-100 whitespace-nowrap'
  ...
  const cancelBtnCls = 'flex-1 py-2 border border-gray-300 dark:border-zinc-700 text-gray-700 dark:text-gray-300 rounded-lg font-medium hover:bg-gray-50 dark:hover:bg-zinc-800 transition-colors'

  const years = Array.from({ length: 5 }, (_, i) => now.getFullYear() - 2 + i)

  return (
    <div className="min-w-0">
      <h2 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-gray-100 mb-4 sm:mb-6">Fechamento Mensal</h2>

      {/* Seletor Mês/Ano */}
      <div className="flex flex-wrap items-center gap-3 mb-6">
        <select value={mes} onChange={e => setMes(parseInt(e.target.value))} className="w-full sm:w-auto px-3 py-2 border border-gray-300 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white rounded-lg text-sm focus:outline-none focus:ring-primary">
          {MONTHS.map((m, i) => <option key={i+1} value={i+1}>{m}</option>)}
        </select>
        <select value={ano} onChange={e => setAno(parseInt(e.target.value))} className="w-full sm:w-auto px-3 py-2 border border-gray-300 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white rounded-lg text-sm focus:outline-none focus:ring-primary">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-3 mb-6">
        {[
          ...
          { label: 'Pizzas', value: `${totalPizzas}`, color: 'text-gray-700 dark:text-gray-300', sub: `Ticket R$ ${formatBRL(ticketMedio)}` },
        ].map(card => (
          <div key={card.label} className="bg-white dark:bg-zinc-900 rounded-xl border border-gray-100 dark:border-zinc-800 p-3 shadow-sm min-w-0">
      ...
      {/* ─── Abas ─── */}
      <div className="flex gap-0.5 -mx-3 sm:mx-0 px-3 sm:px-0 mb-4 border-b border-gray-200 dark:border-zinc-800 overflow-x-auto">
        {(['vendas', 'funcionarios', 'contas', 'insumos'] as Tab[]).map(t => {
          ...
          return (
            <button key={t} onClick={() => setTab(t)} className={`px-3 py-2 text-xs sm:text-sm font-medium border-b-2 whitespace-nowrap transition-colors ${tab === t ? 'border-primary text-primary' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}`}>
              {labels[t]}
            </button>
          )
        })}
      ...
      {tab === 'vendas' && (
        <div>
          <div className="bg-white dark:bg-zinc-900 rounded-xl border border-gray-100 dark:border-zinc-800 p-4 sm:p-5 mb-5 shadow-sm">
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-xl border border-gray-100 dark:border-zinc-800 overflow-hidden shadow-sm">
            <div className="overflow-x-auto">
              <table className="min-w-[980px] divide-y divide-gray-100 dark:divide-gray-700">
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                  {vendas.map(v => {
                    ...
                    return (
                      <tr key={v.id} className="hover:bg-gray-50 dark:hover:bg-zinc-800/60">
                        ...
                        {fechado ? (
                          <td colSpan={9} className="px-3 py-3 text-xs text-gray-400 dark:text-gray-500">
                  ...
                  {vendas.length === 0 && (
                    <tr><td colSpan={12} className="px-4 py-8 text-center text-sm text-gray-400 dark:text-gray-500">Nenhuma venda registrada.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
      ...
      {tab === 'funcionarios' && (
        <div>
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-xl border border-gray-100 dark:border-zinc-800 overflow-hidden shadow-sm">
            <div className="overflow-x-auto">
              <table className="min-w-[680px] divide-y divide-gray-100 dark:divide-gray-700">
                <thead className="bg-gray-50 dark:bg-zinc-800">
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                  ...
                  {funcionarios.length === 0 && <tr><td colSpan={5} className="px-4 py-8 text-center text-sm text-gray-400 dark:text-gray-500">Nenhum pagamento registrado.</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
      ...
      {tab === 'contas' && (
        <div>
            ...
            <form onSubmit={handleContaSubmit}>
              ...
              <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-4 mb-4">
                <div className="flex flex-col min-[420px]:flex-row min-[420px]:items-center gap-2">
                  <label className="text-sm text-gray-600 dark:text-gray-400">Vence todo dia:</label>
                  <select
                    value={(contaForm as any).diaVencimento || ''}
                    onChange={e => setContaForm({ ...contaForm, ...(contaForm as any), diaVencimento: e.target.value || null })}
                    className="w-full min-[420px]:w-auto px-3 py-1.5 border border-gray-300 dark:border-zinc-700 dark:bg-zinc-900 dark:text-white rounded-lg text-sm focus:outline-none focus:ring-primary"
                  >
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-sm text-gray-700 dark:text-gray-300">Marcar como pago</span>
                </label>
              </div>
              <button type="submit" disabled={submitting} className="w-full sm:w-auto px-5 py-2 bg-primary text-white rounded-lg text-sm font-medium hover:bg-primary-dark disabled:opacity-50 transition-colors">
                {submitting ? 'Salvando...' : 'Adicionar'}
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-xl border border-gray-100 dark:border-zinc-800 overflow-hidden shadow-sm">
            <div className="overflow-x-auto">
              <table className="min-w-[800px] divide-y divide-gray-100 dark:divide-zinc-800">
                <thead className="bg-gray-50 dark:bg-zinc-800">
      ...
      {tab === 'insumos' && (
        <div>
          <div className="bg-white dark:bg-zinc-900 rounded-xl border border-gray-100 dark:border-zinc-800 p-4 sm:p-5 mb-5 shadow-sm">
            <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-4">Novo Insumo / Compra</h3>
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-xl border border-gray-100 dark:border-zinc-800 overflow-hidden shadow-sm">
            <div className="overflow-x-auto">
              <table className="min-w-[620px] divide-y divide-gray-100 dark:divide-gray-700">
                <thead className="bg-gray-50 dark:bg-zinc-800">
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                  ...
                  {insumos.length === 0 && <tr><td colSpan={4} className="px-4 py-8 text-center text-sm text-gray-400 dark:text-gray-500">Nenhum insumo registrado.</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
