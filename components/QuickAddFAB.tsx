...
export default function QuickAddFAB() {
  ...
  function reset() {
    ...
    setAvista(0); setDebito(0); setCredito(0); setPix(0); setIfood(0); setOutros(0); setTaxas(0); setPizzas(0)
    ...
    <>
      ...
      <BottomSheet open={open} onClose={() => { setOpen(false); reset() }} title="Novo Registro">
          ...
          {cats.map(c => (
            <button key={c.key} type="button" onClick={() => setCat(c.key)}
              className={`px-3.5 py-1.5 rounded-full text-sm font-medium border transition-colors ${
                cat === c.key ? 'bg-primary text-white border-primary' : 'border-gray-200 dark:border-zinc-700 text-gray-500 dark:text-zinc-400 hover:border-primary hover:text-primary'
              }`}>
              {c.label}
            </button>
          ))}
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          {cat === 'funcionario' && (
            <div>
              <label className="text-xs text-gray-500 dark:text-zinc-400 mb-1 block">Semana / Referência</label>
          ...
          <button type="submit" disabled={submitting || (cat !== 'venda' && valor === 0)}
